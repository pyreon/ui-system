...
const hasAnyValue = (vals: EdgeValues) =>
  isValidValue(vals.top) || isValidValue(vals.bottom) ||
  isValidValue(vals.left) || isValidValue(vals.right) ||
  isValidValue(vals.x) || isValidValue(vals.y) || isValidValue(vals.full)

const resolveSides = ({ full, x, y, top, left, right, bottom }: EdgeValues) => {
  const sides: Value[] = [full, full, full, full]
  if (isValidValue(x)) { sides[1] = x; sides[3] = x }
  if (isValidValue(y)) { sides[0] = y; sides[2] = y }
...
const formatIndividual = (sides: Value[], edgeCss: (side: Side) => string, calc: (v: Value) => Value) => {
  const [t, r, b, l] = sides
  let output = ''
...
const edge: Edge = (rootSize = 16) => (property, values) => {
