import type { BlurOptions, CSSProperties, Direction, FadeOptions, Preset, RotateOptions, ScaleOptions, SlideOptions } from './types'
...
export const createFade = ({ direction, distance = 16, duration = 300, leaveDuration = 200, easing = 'ease-out', leaveEasing = 'ease-in' }: FadeOptions = {}): Preset => {
  if (!direction) {
    return {
...
export const createSlide = ({ direction = 'up', distance = 16, duration = 300, leaveDuration = 200, easing = 'ease-out', leaveEasing = 'ease-in' }: SlideOptions = {}): Preset => {
  const translate = directionToTranslate(direction, distance)
  const zero = directionToZero(direction)
  return {
    enterStyle: { opacity: 0, transform: translate },
...
export const createScale = ({ from = 0.9, duration = 300, leaveDuration = 200, easing = 'ease-out', leaveEasing = 'ease-in' }: ScaleOptions = {}): Preset => ({
...
export const createRotate = ({ degrees = 15, duration = 300, leaveDuration = 200, easing = 'ease-out', leaveEasing = 'ease-in' }: RotateOptions = {}): Preset => ({
...
export const createBlur = ({ amount = 8, scale, duration = 300, leaveDuration = 200, easing = 'ease-out', leaveEasing = 'ease-in' }: BlurOptions = {}): Preset => {
