...
const wireRef = (vnode: VNode | null, el: HTMLElement) => {
  ...
  const visitNode = (node: VNode) => {
    ...
    if (typeof nodeProps?.ref === 'function') {
      ;(nodeProps.ref as (el: HTMLElement | null) => void)(el)
    ...
    if (nodeProps?.fallback && typeof nodeProps.fallback === 'object' && 'type' in (nodeProps.fallback as object)) {
...
describe('Transition', () => {
  it('returns a VNode when show=true', () => {
    ...
    const child: VNode = { type: 'div', props: {}, children: ['Hello'], key: undefined }
  ...
  it('fires onEnter callback when entering starts', () => {
    ...
    const { el } = setupTransition({ show, onEnter })
  ...
  it('fires onLeave callback when leaving starts', () => {
    ...
    const { el } = setupTransition({ show, onLeave })
  ...
  it('timeout fallback completes transition when transitionend never fires', () => {
    ...
    const { el } = setupTransition({ show, timeout: 1000, onAfterEnter })
