...
const wireRef = (vnode: VNode | null, el: HTMLElement) => {
  ...
  const visitNode = (node: VNode) => {
    ...
    if (typeof props?.ref === 'function') {
      ;(props.ref as (el: HTMLElement | null) => void)(el)
    ...
    if (props?.fallback && typeof props.fallback === 'object' && 'type' in (props.fallback as object)) {
...
describe('kinetic() — transition mode', () => {
  ...
  it('fires lifecycle callbacks at correct times', () => {
    ...
    const FadeDiv = kinetic('div').preset(fade)
    const el = document.createElement('div')
    const vnode = resolveComponent(FadeDiv({ show, onEnter, onAfterEnter, onLeave, onAfterLeave, children: 'Hello' }))
    wireRef(vnode, el)
...
describe('kinetic() — collapse mode', () => {
  ...
  it('fires onEnter on entering and onAfterEnter after transitionend', () => {
    ...
    const vnode = resolveComponent(Accordion({ show, onEnter, onAfterEnter, children: { type: 'p', props: {}, children: ['Content'], key: undefined } }))
