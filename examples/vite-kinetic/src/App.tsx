...
import { kinetic, fade, scaleIn, slideUp, slideDown, slideLeft, slideRight } from '@pyreon/kinetic'
import {
  presets,
  createFade, createSlide, createScale, createRotate, createBlur,
...
const StaggerNotifications = kinetic('div').preset(fade).stagger({ interval: 100, reverseLeave: true })
const StaggerCardGrid = kinetic('div').preset(scaleIn).stagger({ interval: 50 })
...
const FactoryScaleSpring = kinetic('div').preset(createScale({ from: 0.5, easing: 'cubic-bezier(0.34, 1.56, 0.64, 1)' }))
...
const SpringEased = kinetic('div').preset(withEasing(presets.scaleIn, 'cubic-bezier(0.34, 1.56, 0.64, 1)'))
...
const presetComponents: Record<string, ReturnType<typeof kinetic>> = Object.fromEntries(
  Object.entries(presets).map(([name, preset]) => [
    name,
    kinetic('div').preset(preset as Preset),
  ]),
)

// Preset categories for gallery
const PRESET_CATEGORIES = [
  { name: 'Fades', items: ['fade', 'fadeUp', 'fadeDown', 'fadeLeft', 'fadeRight', 'fadeUpBig', 'fadeDownBig', 'fadeLeftBig', 'fadeRightBig', 'fadeScale', 'fadeUpLeft', 'fadeUpRight', 'fadeDownLeft', 'fadeDownRight'] },
  { name: 'Slides', items: ['slideUp', 'slideDown', 'slideLeft', 'slideRight', 'slideUpBig', 'slideDownBig', 'slideLeftBig', 'slideRightBig'] },
  { name: 'Scales', items: ['scaleIn', 'scaleOut', 'scaleUp', 'scaleDown', 'scaleInUp', 'scaleInDown', 'scaleInLeft', 'scaleInRight'] },
  { name: 'Zooms', items: ['zoomIn', 'zoomOut', 'zoomInUp', 'zoomInDown', 'zoomInLeft', 'zoomInRight', 'zoomOutUp', 'zoomOutDown', 'zoomOutLeft', 'zoomOutRight'] },
  { name: 'Flips', items: ['flipX', 'flipY', 'flipXReverse', 'flipYReverse', 'flipDiagonal', 'flipDiagonalReverse'] },
  { name: 'Rotations', items: ['rotateIn', 'rotateInReverse', 'rotateInUp', 'rotateInDown', 'spinIn', 'spinInReverse', 'scaleRotateIn', 'newspaperIn'] },
  { name: 'Bounce & Spring', items: ['bounceIn', 'bounceInUp', 'bounceInDown', 'bounceInLeft', 'bounceInRight', 'springIn', 'popIn', 'rubberIn', 'squishX', 'squishY'] },
  { name: 'Blur', items: ['blurIn', 'blurInUp', 'blurInDown', 'blurInLeft', 'blurInRight', 'blurScale'] },
  { name: 'Puff', items: ['puffIn', 'puffOut'] },
  { name: 'Clip Path', items: ['clipTop', 'clipBottom', 'clipLeft', 'clipRight', 'clipCircle', 'clipCenter', 'clipDiamond', 'clipCorner'] },
  { name: 'Perspective', items: ['perspectiveUp', 'perspectiveDown', 'perspectiveLeft', 'perspectiveRight'] },
...
function PresetShowcase() {
  ...
  const components = { fade: FadeDiv, scaleIn: ScaleInDiv, slideUp: SlideUpDiv, slideDown: SlideDownDiv, slideLeft: SlideLeftDiv, slideRight: SlideRightDiv }
...
function StyleObjectDemo() {
  ...
  return (
    <div style={card}>
      ...
      <button style={btn} onClick={() => show.update((v) => !v)}>Toggle</button>
...
function UnmountDemo() {
  ...
  return (
    <div style={card}>
      ...
      <div style={gridRow}>
        <div style={{ flex: '1' }}>
          <button style={btn} onClick={() => showUnmount.update((v) => !v)}>Toggle unmount</button>
          <div style={{ 'margin-top': '12px' }}>
            <FadeDiv show={() => showUnmount()} style={colorBox}>Unmount (removed from DOM)</FadeDiv>
          </div>
        </div>
        <div style={{ flex: '1' }}>
          <button style={btn} onClick={() => showHide.update((v) => !v)}>Toggle hide</button>
          <div style={{ 'margin-top': '12px' }}>
            <FadeDiv show={() => showHide()} unmount={false} style={colorBox}>Hide (display:none)</FadeDiv>
          </div>
        </div>
      </div>
    </div>
...
function LifecycleDemo() {
  ...
  return (
    <div style={card}>
      ...
      <button style={btn} onClick={() => show.update((v) => !v)}>Toggle</button>
      <button style={{ ...btn, 'margin-left': '8px' }} onClick={() => logs.set([])}>Clear log</button>
      <div style={{ 'margin-top': '16px' }}>
        <FadeDiv
          show={() => show()}
...
function ChainingDemo() {
  ...
  const Base = kinetic('div').enter({ opacity: 0 }).enterTo({ opacity: 1 }).enterTransition('opacity 300ms ease-out')
  const WithSlide = Base.enter({ opacity: 0, transform: 'translateY(20px)' }).enterTo({ opacity: 1, transform: 'translateY(0)' }).enterTransition('all 300ms ease-out')
  const WithScale = Base.enter({ opacity: 0, transform: 'scale(0.8)' }).enterTo({ opacity: 1, transform: 'scale(1)' }).enterTransition('all 400ms cubic-bezier(0.34, 1.56, 0.64, 1)')
  ...
  return (
    <div style={card}>
      ...
      <button style={btn} onClick={() => show.update((v) => !v)}>Toggle all</button>
      <div style={{ display: 'flex', gap: '12px', 'margin-top': '16px' }}>
        <div style={{ flex: '1' }}>
          <Base show={() => show()} style={colorBox}>Base (fade)</Base>
        </div>
        <div style={{ flex: '1' }}>
          <WithSlide show={() => show()} style={colorBox}>+ Slide</WithSlide>
        </div>
        <div style={{ flex: '1' }}>
          <WithScale show={() => show()} style={colorBox}>+ Scale</WithScale>
        </div>
      </div>
    </div>
  )
...
function CollapseDemo() {
  ...
  const items = [
    { title: 'What is Kinetic?', content: 'A CSS-first animation library for Pyreon. Enter/exit transitions, stagger, collapse, and list reconciliation in ~3KB.' },
    { title: 'How does it work?', content: 'Kinetic delegates interpolation to the CSS transition engine (compositor thread for transform/opacity) and handles orchestration — mount/unmount lifecycle, stagger timing, and height measurement.' },
    { title: 'What about accessibility?', content: 'Kinetic automatically detects prefers-reduced-motion: reduce. When enabled, animations are skipped instantly — callbacks still fire, but no visual animation occurs.' },
    { title: 'Can I create custom presets?', content: 'Yes! A preset is just an object with enter/leave styles and transitions. You can also use factories (createFade, createSlide, etc.) for parameterized presets.' },
  ]

  return (
    <div style={card}>
      <div style={cardTitle}>Collapse / Accordion</div>
      <div style={cardDesc}>Height-based animation with overflow:hidden. Measures scrollHeight automatically.</div>
      {items.map((item, i) => (
        <div>
          <div
...
function CollapseCustomDemo() {
  ...
  return (
    <div style={card}>
      ...
      <button style={btn} onClick={() => open.update((v) => !v)}>
        {() => open() ? 'Collapse' : 'Expand'}
      </button>
      <div style={{ 'margin-top': '12px' }}>
        <BouncyCollapse show={() => open()}>
          <div style={accordionContent}>
            This content bounces into view with a spring-like cubic-bezier curve.
            The height transition uses <code>cubic-bezier(0.34, 1.56, 0.64, 1)</code>
...
function GroupDemo() {
  ...
  return (
    <div style={card}>
      ...
      <div style={cardDesc}>
        Adding a child triggers enter animation. Removing triggers leave + unmount. No show prop needed.
      </div>
      <div style={{ display: 'flex', gap: '8px', 'margin-bottom': '16px' }}>
        <button style={btnPrimary} onClick={addItem}>Add Item</button>
        <button style={btn} onClick={shuffle}>Shuffle</button>
        <button style={btnDanger} onClick={() => items.set([])}>Clear All</button>
      </div>
      <GroupList>
        {() => items().map((item) => (
          <div key={item.id} style={listItem}>
            <span>{item.text}</span>
            <button style={btn} onClick={() => removeItem(item.id)}>Remove</button>
          </div>
        ))}
      </GroupList>
    </div>
...
function StaggerDemo() {
  ...
  return (
    <div style={card}>
      ...
      <button style={btn} onClick={() => show.update((v) => !v)}>
    ...
    <div style={card}>
      ...
      <div style={{ 'margin-top': '16px' }}>
        <StaggerNotifications show={() => show()}>
          {['New message received', 'File uploaded successfully', 'Settings saved'].map((msg, i) => (
            <div key={i} style={{ ...listItem, background: '#f0f7ff', 'border-color': '#b8daff' }}>
              {msg}
            </div>
          ))}
...
function ModalDemo() {
  ...
  return (
    <div style={card}>
      ...
      <div style={cardDesc}>
        Backdrop (fade) + Dialog (spring scale) composed together.
      </div>
      <button style={btnPrimary} onClick={() => open.set(true)}>Open Modal</button>
      <Backdrop
        show={() => open()}
        style={{
          position: 'fixed',
      ...
      >
        ...
        >
          ...
          <p style={{ color: '#555', margin: '0 0 24px', 'line-height': '1.6' }}>
            This modal uses a fade backdrop and a spring-scaled dialog.
            Click outside or the button below to close.
          </p>
          <button style={btnPrimary} onClick={() => open.set(false)}>Close</button>
        </Dialog>
      </Backdrop>
    </div>
  )
...
function ToastDemo() {
  ...
  const addToast = () => {
    const id = toastId++
    const messages = ['File saved successfully', 'New notification', 'Upload complete', 'Settings updated']
  ...
  return (
    <div style={card}>
      ...
      <button style={btnPrimary} onClick={addToast}>Add Toast</button>
      <div style={{ position: 'relative', 'min-height': '100px', 'margin-top': '16px' }}>
        <ToastGroup>
          {() => toasts().map((toast) => (
            <div
              key={toast.id}
...
function TabsDemo() {
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: '0', 'margin-bottom': '16px', 'border-bottom': '2px solid #e9ecef' }}>
        {tabs.map((tab, i) => (
          <button
            style={() => ({
              padding: '8px 20px',
              ...
              'font-size': '14px',
              'font-weight': '500',
              'border-bottom': activeTab() === i ? '2px solid #0070f3' : '2px solid transparent',
      ...
      {content.map((text, i) => (
        <FadeDiv show={() => activeTab() === i}>
          <div style={{ padding: '16px', color: '#555', 'line-height': '1.6' }}>{text}</div>
        </FadeDiv>
      ))}
    </div>
  )
        ...
        {factories.map(({ name, comp: Comp, desc }) => (
          <div style={{ flex: '1', 'min-width': '160px' }}>
            ...
            <Comp show={() => show()} style={colorBox}>
              ...
              <div style={{ 'font-size': '11px', 'margin-top': '4px', opacity: '0.8' }}>{desc}</div>
            </Comp>
          </div>
        ))}
      </div>
...
function PresetCard(props: { name: string }) {
  ...
  return (
    <div style={presetCardStyle}>
      ...
      <button
        style={{ ...btn, 'font-size': '11px', padding: '4px 10px', 'margin-bottom': '8px' }}
...
function KineticPresetsGallery() {
  ...
  return (
    <div style={card}>
      ...
      <div style={cardDesc}>
        Click a category to expand and toggle individual presets.
      </div>
...
function ToggleAllPresets() {
  ...
  return (
    <div style={card}>
      ...
      <button style={btnPrimary} onClick={() => show.update((v) => !v)}>
...
export function App() {
  return (
    <div style={wrapper}>
      ...
      <h1 style={{ 'font-size': '2rem', 'font-weight': '700', margin: '8px 0 4px' }}>
        Kinetic Animation Examples
      </h1>
      <p style={{ 'font-size': '1rem', color: '#666', margin: '0 0 32px' }}>
